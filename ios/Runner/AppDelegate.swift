...
channel.setMethodCallHandler { (call, result) in
    ...
    dateComponents.weekday = weekday // 1 = Chủ nhật, 2 = Thứ 2, ..., 7 = Thứ 7
    ...
    UNUserNotificationCenter.current().add(request) { error in
        if let error = error {
            ...
            print("✅ Weekly notification set for weekday \(weekday) at \(hour):\(minute)")
        }
    }
}
    ...
    func scheduleDailyNotification(atHour hour: Int, minute: Int) {
        ...
        let request = UNNotificationRequest(identifier: "daily_notification_\(hour)_\(minute)",
                                            content: content,
                                            trigger: trigger)
